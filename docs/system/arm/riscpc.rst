...
``STRH`` transfers.  It is a memory-bus limitation: cached accesses can work,
while uncached accesses are unreliable (and stores are particularly unsafe).
QEMU does not yet model the StrongARM cache/write-buffer traffic needed to
decide that per transfer.  Its deliberately broad diagnostic mode preserves
address-fault behaviour, supplies an unpredictable data value and logs each
execution with ``-d guest_errors``.  Enable it explicitly with
``-M riscpc,broken-halfword=on``; it is RiscPC-only, so an SA-110 in another
machine such as the NetWinder keeps normal ARMv4 halfword transfers.  Normal
RiscPC emulation leaves this provisional mode off until the bus behaviour is
fully characterized.
